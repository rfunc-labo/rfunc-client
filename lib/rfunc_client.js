...
class RFuncClint {
  ...
  connect (moduleName) {
    ...
    return co(function * () {
      ...
      let { name, version, methods } = yield s.describe(moduleName)

      let api = methods.default ? (...args) => api.default(...args) : {}
      Object.assign(
        api,
        {
          then: null,
  ...
  invoke (moduleName, methodName, ...params) {
    ...
    return co(function * () {
      let { body, statusCode } = yield agent
        ...
        .use(superagentPromisePlugin)
        .catch((thrown) => {
          let { status, response } = thrown
          let { errors = [] } = (response && response.body || {})
          let [ error = thrown ] = errors
          let {
            name,
